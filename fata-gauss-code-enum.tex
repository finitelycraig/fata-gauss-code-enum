...

{
    \usebackgroundtemplate{
        ...
        \node[at=(current page.south), anchor=south, inner sep=0pt]{\includegraphics*[keepaspectratio=true, width=\paperwidth]{3_1.png}};
    }
...
\begin{knot}[
 ...
 background color=uofgheather,
]
\strand
(90:2) to[out=180,in=-120,looseness=3]
(-30:2) to[out=60,in=120,looseness=2]
(210:2) to[out=-60,in=0,looseness=1] (90:2);
\end{knot}
\end{tikzpicture}
\end{frame}
...
\begin{frame}
    \begin{tikzpicture}
        \draw[red, ultra thick] (0,0) .. controls +(1,0) and
            +(-1,0) .. (2,1) .. controls +(1,0) and +(-1,0) ..
            (4,0);
        \draw [blue , ultra thick] (0,1) .. controls +(1,0) and
            +(-1,0) .. (2,0) .. controls +(1,0) and +(-1,0) ..
            (4,1);
    \end{tikzpicture}
\end{frame}
...
\draw[thick,uofgpumpkin] (k\brk) .. controls (k\brk.4 north west) and (k\brl.4 north east) .. (k\brl.center);

\draw[thick,uofgheather] (k\brk.center) .. controls (k\brk.16 south west) and (k\brl.16 south east) .. (k\brl);
}
\end{tikzpicture}
\end{frame}
